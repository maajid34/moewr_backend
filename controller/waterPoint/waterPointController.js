...
exports.create = async (req, res) => {
  const create = () =>
    transaction(async (session) => {
      ...
      // Water Point code prefix based on source type
      const prefixes = {
        BOREHOLE: "BH",
        SHALLOW_WELL: "SW",
        BARKAD: "BK",
      ...
      const prefix = prefixes[b.waterSourceType];

      if (!prefix) {
        error(400, "Invalid water source type");
      }
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      if (
        attempt === 4 ||
        err.code !== 11000 ||
        !["BH", "SW", "BK", "WP", "WK"].includes(err.keyValue?._id)
      ) {
        throw err;
      }
    }
...
exports.get = async (req, res) => {
  const doc = await populateLocation(
    WaterPoint.findById(req.params.id),
  ).lean();
...
exports.update = async (req, res) => {
  const result = await transaction(async (session) => {
    ...
    if (!doc.isActive) {
      error(409, "Archived water point cannot be changed");
    }
